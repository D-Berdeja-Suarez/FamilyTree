...
########################################################### Person Class ###############################################
# Remember: we inherit from object so we can pass this custom class as a parameter in a QSignal.
class Person(object):
    """
...
class WelcomeScreen(qtw.QMainWindow, Ui_welcomescreen):
    ...
    def _new_pressed(self):
        """
        Slot for New pushbutton.
        :return:
        """

        filename = self.lineEdit.text() + '.db'

        # If file already exists, we raise a warning dialogue.
        if file_exists(filename):
            ...
            overwrite_dialog.accepted.connect( lambda: self._create_new_tree( filename = filename ) )


        else:

            self._create_new_tree(filename=filename)

    def _create_new_tree(self, filename):

        self._person_input_screen = PersonInputScreen()

        self._person_input_screen.person_signal.connect( lambda x: self._open_tree_viewer(
                                                                    tree=FamilyTree(root = x),
                                                                    filename=filename ) )
        self._person_input_screen.show()
...
class PersonInputScreen(qtw.QMainWindow, Ui_personinputscreen):
    ...
    # Remember: we must specify the classes that can be passed as parameters.
    ...
    def _done_pressed(self):
        """
        Collects personal information and returns a Person instance. Emits signal with Person instance if successful.
        :return: 0 if not enough information is provided; 1 if collection is successful.
        """
